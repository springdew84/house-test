...
final class WebP {
  private static Logger logger = LoggerFactory.getLogger(WebP.class);
  private static boolean NATIVE_LIBRARY_LOADED = false;

  static synchronized void loadNativeLibrary() {
    if (!NATIVE_LIBRARY_LOADED) {
      NATIVE_LIBRARY_LOADED = true;
      boolean loadRes = NativeLibraryUtil.loadNativeLibrary(WebP.class, "webp-imageio");
      if (loadRes) {
        logger.info("loaded webp-imageio");
      } else {
        logger.info("loaded webp-imageio lib failure!");
      }
    }
  }
