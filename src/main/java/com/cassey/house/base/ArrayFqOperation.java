...
public class ArrayFqOperation {

    public static void main(String[] args) {
        //二维数组
        //第一种：
        //int[][] biarr1 = new int[][]{{1,2}, {2, 3}, {4, 5}};
        int[][] biarr1 = {{1,2}, {2, 3}, {4, 5}};
        System.out.println("arr1的数值：");
        for(int i=0; i<3; ++i) {
            for(int j=0; j<2; ++j)
                System.out.print(biarr1[i][j]);
            System.out.print("\n");//显示成数组的表现形式
        }

        //第二种
        int[][] biarr2 = new int[4][3];
        ...
        for(int i=0; i<biarr2.length; ++i) {
            for(int j=0; j<biarr2[i].length; ++j)
                System.out.print(biarr2[i][j]);
            System.out.print("\n");
        }

        //第三种
        int[][] biarr3 = new int[4][];//五行的长度
        System.out.println("arr3的数值：");
        for(int i=0; i<biarr3.length; ++i) {
            biarr3[i] = new int[i + 1];
            //列的长度每次都变化。每次都要重新申请空间(长度)
            for (int j = 0; j < biarr3[i].length; ++j)
                biarr3[i][j] = i + j;
        }
        for(int i=0; i<biarr3.length; ++i) {
            for(int j=0; j<biarr3[i].length; ++j)
                System.out.print(biarr3[i][j]);
            System.out.print("\n");
        }
